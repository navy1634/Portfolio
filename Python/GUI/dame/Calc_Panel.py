...
columns = ["味方", "敵", "技名", "最低ダメージ", "最高ダメージ", "最低ダメージ割合", "最高ダメージ割合", "味方レベル", "味方特性", "味方攻撃実数値", "味方性格", "味方持ち物", "味方ダイマックス", "敵レベル", "敵特性", "敵HP実数値", "敵性格", "敵持ち物", "敵ダイマックス"]
...
class PK_calc_Panel(wx.Panel):
    def __init__(self, parent):
        ...
        # 図鑑で追加したポケモンのリスト
        self.pk_list = parent.Pic_Panel.pk_list
        if self.pk_list == []:
    ...
    def set_output_data(self):
        ...
        self.output_data.append(self.damage_M)
        self.output_data.append(round(self.damage_m / self.H_correct_enem * 100, 2))
        self.output_data.append(round(self.damage_M / self.H_correct_enem * 100, 2))
    ...
    def pk_use_move_list(self, event):
        ...
        if toggle == True:
            ...
            for move in use_move_list:
                if use_move_list_my[move] != "過去作限定":
                    self.move_box.Append(move)
    ...
    def calc_B_status_my(self, event):
        ...
        if (0 <= B_indi_my <= 31) and (0 <= B_effort_my <=  255) and (self.sum_effort_my.GetValue() != "Error"):
            ...
            self.B_correct_my = floor(floor(self.B_real_value_my * self.calc_char(st="A", char=self.char)) * self.rank_rate(B_rank_my))
    ...
    def calc_C_status_my(self, event):
        ...
        if (0 <= C_indi_my <= 31) and (0 <= C_effort_my <=  255) and (self.sum_effort_my.GetValue() != "Error"):
            ...
            self.C_correct_my = floor(floor(self.C_real_value_my * self.calc_char(st="A", char=self.char)) * self.rank_rate(C_rank_my))
    ...
    def calc_D_status_my(self, event):
        ...
        if (0 <= D_indi_my <= 31) and (0 <= D_effort_my <=  255) and (self.sum_effort_my.GetValue() != "Error"):
            ...
            self.D_correct_my = floor(floor(self.D_real_value_my * self.calc_char(st="A", char=self.char)) * self.rank_rate(D_rank_my))
    ...
    def calc_S_status_my(self, event):
        ...
        if (0 <= S_indi_my <= 31) and (0 <= S_effort_my <=  255) and (self.sum_effort_my.GetValue() != "Error"):
            ...
            self.S_correct_my = floor(floor(self.S_real_value_my * self.calc_char(st="A", char=self.char)) * self.rank_rate(S_rank_my))
    ...
    def calc_B_status_enem(self, event):
        ...
        if (0 <= B_indi_enem <= 31) and (0 <= B_effort_enem <=  255) and (self.sum_effort_enem.GetValue() != "Error"):
            ...
            self.B_correct_enem = floor(floor(self.B_real_value_enem * self.calc_char(st="A", char=self.char)) * self.rank_rate(B_rank_enem))
    ...
    def calc_C_status_enem(self, event):
        ...
        if (0 <= C_indi_enem <= 31) and (0 <= C_effort_enem <=  255) and (self.sum_effort_enem.GetValue() != "Error"):
            ...
            self.C_correct_enem = floor(floor(self.C_real_value_enem * self.calc_char(st="A", char=self.char)) * self.rank_rate(C_rank_enem))
    ...
    def calc_D_status_enem(self, event):
        ...
        if (0 <= D_indi_enem <= 31) and (0 <= D_effort_enem <=  255) and (self.sum_effort_enem.GetValue() != "Error"):
            ...
            self.D_correct_enem = floor(floor(self.D_real_value_enem * self.calc_char(st="A", char=self.char)) * self.rank_rate(D_rank_enem))
    ...
    def calc_S_status_enem(self, event):
        ...
        if (0 <= S_indi_enem <= 31) and (0 <= S_effort_enem <=  255) and (self.sum_effort_enem.GetValue() != "Error"):
            ...
            self.S_correct_enem = floor(floor(self.S_real_value_enem * self.calc_char(st="A", char=self.char)) * self.rank_rate(S_rank_enem))
    ...
    def effort_layout_my(self):
        ...
        real_correct = wx.StaticText(self, -1, "補正込実数値", size=self.size)
    ...
    def pk_layout_my(self):
        ...
        self.ability_box_my = wx.ComboBox(self, -1, "特性", size=self.size, choices=[])
